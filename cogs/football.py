...
class PredictModal(ui.Modal, title="Dự đoán tỷ số trận đấu"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        lock_time = self.fixture["kickoff"] + PREDICTION_LOCK_AFTER
        if now >= lock_time:
            await interaction.response.send_message(
                "Đã quá 5 phút kể từ khi trận đấu bắt đầu. Không thể dự đoán nữa.",
                ephemeral=True,
            )
            return
...
class Football(commands.Cog):
    ...
    async def predict_cmd(self, ctx: commands.Context, *, match_number: int = None):
        ...
        now = datetime.now(timezone.utc)
        lock_time = fixture["kickoff"] + PREDICTION_LOCK_AFTER
        if now >= lock_time:
            await ctx.send(
                f"{ctx.author.mention}, đã quá 5 phút kể từ khi trận đấu bắt đầu. Không thể dự đoán nữa."
            )
            return
    ...
    @commands.command(name="showwc")
    async def show_predictions(self, ctx: commands.Context, *, match_number: int = None):
        """Xem dự đoán của mọi người cho một trận đấu.
        ...
        if match_number is None or not (1 <= match_number <= len(upcoming)):
            embed = self._build_upcoming_embed(upcoming)
            embed.set_footer(
                text="Dùng !showwc <số> để xem dự đoán | Source: football-data.org"
            )
        ...
        fixture = upcoming[match_number - 1]
        fid = fixture["id"]
        preds = self._predictions.get(fid, {})

        kickoff_local = fixture["kickoff"].astimezone(LOCAL_TZ)

        if not preds:
            embed = discord.Embed(
                title=f"⚽ Dự đoán: {fixture['home']} vs {fixture['away']}",
                description="Chưa có ai dự đoán trận này.",
        ...
        lines = []
        for idx, (user_id, (h, a)) in enumerate(preds.items(), start=1):
            member = ctx.guild.get_member(user_id) if ctx.guild else None
            name = member.display_name if member else f"User#{user_id}"
            lines.append(f"`{idx}.` **{name}** — {fixture['home']} **{h}** - **{a}** {fixture['away']}")
